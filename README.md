...
| [tubewalk](https://github.com/DigitalCurrensy/tubewalk) | Tube line gate | Is the line a tunnel | Width, length, echo, clutter | `dark` if the echo is missing, blank, or `none` in any case. A non-text echo is `missing`. Then `missing` for a blank, negative, or non-finite width or length. Then `pinch` under 10 m, `stub` under 30 m, `clutter`, else `ok`. Zero is a number. `ok` is not a ceiling |
